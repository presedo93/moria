...
use moria_common::config::RiskConfig;
...
async fn main() -> Result<()> {
    ...
    moria_common::telemetry::init_metrics("risk", config.telemetry.metrics_addr.as_deref())?;

    // Connect to PostgreSQL with retry
    // Set statement_timeout at connection level to prevent indefinite query blocking
    let connect_options = PgConnectOptions::from_str(&config.db.database_url)?
    ...
    worker::spawn_order_execution_worker(
        pool.clone(),
        config.db.database_url.clone(),
        order_client.clone(),
        config.auth.internal_service_token.clone(),
    );
